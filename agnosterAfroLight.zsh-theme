...
prompt_battery() {
  ...
  if [[ $(uname) == "Linux"  ]] ; then
    ...
    if [[ $(acpi 2&>/dev/null | grep -c '^Battery.*Discharging') -gt 0 ]] ; then
      ...
      prompt_segment green white
      echo -n "%{$fg_bold[white]%}$THUNDER$(battery_pct_remaining)%% %{$fg_no_bold[white]%}"
    fi
...
prompt_time() {
  prompt_segment blue white "%{$fg_bold[white]%}%D{%a %e %b}%{$fg_no_bold[white]%}"
}
